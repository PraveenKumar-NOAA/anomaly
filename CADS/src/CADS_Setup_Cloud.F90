...
!   31/01/24   C.Burrows   3.2   Add imager options for CrIS/VIIRS
!                                Add functionality for HIRAS-2 and IRIS
...
INTEGER(KIND=4), PARAMETER :: JP__Max_Channels = 16922

INTEGER(KIND=4) :: M__Sensor
INTEGER(KIND=4) :: N__Num_Bands
INTEGER(KIND=4) :: N__GradChkInterval(JP__Max_Bands)
INTEGER(KIND=4) :: N__Band_Size(JP__Max_Bands)
INTEGER(KIND=4) :: N__Bands(JP__Max_Channels,JP__Max_Bands)
INTEGER(KIND=4) :: N__Window_Width(JP__Max_Bands)
INTEGER(KIND=4) :: N__Window_Bounds(JP__Max_Bands,2)
REAL(KIND=8)    :: R__BT_Threshold(JP__Max_Bands)
REAL(KIND=8)    :: R__Grad_Threshold(JP__Max_Bands)
REAL(KIND=8)    :: R__Window_Grad_Threshold(JP__Max_Bands)
LOGICAL         :: L__Do_Quick_Exit
LOGICAL         :: L__Do_CrossBand
INTEGER(KIND=4) :: N__BandToUse(JP__Max_Bands)

! Imager-based cloud detection
LOGICAL         :: L__Do_Imager_Cloud_Detection
INTEGER(KIND=4) :: N__Num_Imager_Chans
INTEGER(KIND=4) :: N__Num_Imager_Clusters
INTEGER(KIND=4) :: N__Imager_Chans(JP__Max_Bands)
REAL(KIND=8)    :: R__Stddev_Threshold(JP__Max_Bands)
REAL(KIND=8)    :: R__Coverage_Threshold
REAL(KIND=8)    :: R__FG_Departure_Threshold
...
WRITE (*,'(A)') ''
WRITE (*,'(A)') 'Setting up the cloud detection ...'
...
&  (/ 1189, 1197, 1205, 1213, 1221, 1251  /)
    ...
    N__Imager_Chans(:) = 0
    N__Imager_Chans(1:N__Num_Imager_Chans) = (/ 5, 7 /)
...
&      457,  481,  501,  549,  701,  705,  709 /)
  ...
  CASE(INST_ID_HIRAS2)
    ...
    CL__Cloud_Detection_File = 'HIRAS2_CLDDET.NL'
...
&      2039, 2042, 2043, 2048, 2049, 2065, 2066, 2069, 2070, 2071 /)
  ...
  CASE(INST_ID_IRIS)
      ...
      CL__Cloud_Detection_File = 'IRIS_CLDDET.NL'

      N__Num_Bands = 1

      N__Band_Size(:) = 0

      N__Band_Size(1:N__NUM_BANDS) =(/ 35 /)
...
&        60,   61,   62,   63,   64,   65 /)
      ...
      N__Window_Width(:) = 0
      N__Window_Width(1:N__Num_Bands) = (/ 5 /)

      N__Window_Bounds(:,:) = 0
      N__Window_Bounds(1,1) = 20
      N__Window_Bounds(1,2) = 100

      R__BT_Threshold(:) = 0.
      R__BT_Threshold(1:N__Num_Bands) = (/ 0.5 /)
