...
USE CADS_Module, ONLY : S__CADS_Setup_Cloud, &
                        INST_ID_AIRS
IMPLICIT NONE
...
INTEGER(KIND=4), INTENT(IN)  :: K__ChanID(:)           ! Channel IDs
INTEGER(KIND=4), INTENT(IN)  :: K__Minlev              ! Highest starting point
                                                       ! for cloud search
INTEGER(KIND=4), INTENT(IN)  :: K__Maxlev              ! Lowest starting point
                                                       ! in the initial search
INTEGER(KIND=4), INTENT(IN)  :: K__Num_Imager_Chans    ! No. of imager channels
INTEGER(KIND=4), INTENT(IN)  :: K__Chan_ID_Imager(:)   ! Imager channel IDs
INTEGER(KIND=4), INTENT(IN)  :: K__Num_Imager_Clusters ! No. of imager clusters
INTEGER(KIND=4), INTENT(OUT) :: K__Cloud_Flag(:)       ! Output cloud flags
REAL(KIND=8),    INTENT(IN)  :: P__ObsBTs(:)           ! Observed BTs
REAL(KIND=8),    INTENT(IN)  :: P__ModelBTs(:)         ! Model clear BTs
REAL(KIND=8),    INTENT(IN)  :: P__Chan_Level(:)       ! Channel height
                                                       ! assignments
REAL(KIND=8),    INTENT(IN)  :: P__Cluster_Fraction(:) ! Cluster coverages
REAL(KIND=8),    INTENT(IN)  :: P__BT_in_Cluster(:,:)  ! Mean BT in cluster / 
                                                       ! channel
REAL(KIND=8),    INTENT(IN)  :: P__BT_Overall_Sdev(:)  ! St.Dev of imager BT
                                                       ! in FOV
REAL(KIND=8),    INTENT(IN)  :: P__BT_Model_Imager(:)  ! Model-based estimate
                                                       ! of imager BT

!* 0.2 local variables
INTEGER(KIND=4)              :: IST,ICOUNT,J,I_K,JBAND,JBAND2
INTEGER(KIND=4)              :: I__Imager_Flag   ! Preliminary cloud flag from
                                                 ! collocated imager data

!* 0.3 Local variables - band splitting details
INTEGER(KIND=4), POINTER     :: I__Bands(:,:)            ! Channel bands
INTEGER(KIND=4), POINTER     :: I__Band_Size(:)          ! Number of channels
                                                         ! per band
INTEGER(KIND=4), POINTER     :: I__BandToUse(:)          ! Cross-band
                                                         ! definitions
INTEGER(KIND=4)              :: I__Num_Bands             ! Number of bands
INTEGER(KIND=4)              :: I__NumFoundChans         ! Number of usable
                                                         ! channels
INTEGER(KIND=4)              :: I__BandNumber(K__NChans) ! Channel band
                                                         ! indicator
INTEGER(KIND=4)              :: I__WindowBounds(2)       ! Boundary of window
INTEGER(KIND=4)              :: I__Window_Chans(2)       ! Boundary of long-wave
                                                         ! window
INTEGER(KIND=4), ALLOCATABLE :: I__INDEX(:)              ! Channel ranking
                                                         ! within a band
INTEGER(KIND=4), ALLOCATABLE :: IDCHAN(:)                ! Overall channel
                                                         ! ranking
INTEGER(KIND=4), ALLOCATABLE :: I__Cloud_Flag(:)         ! Rank-sorted output
                                                         ! cloud flags
INTEGER(KIND=4)              :: I__Scenario_Index        ! 1--Quick Exit
                                                         ! 2--Warm Start
                                                         ! 3--Cold Start
INTEGER(KIND=4)              :: I__Start_Channel         ! Final starting
                                                         ! channel in the
                                                         ! cloud search
...
REAL(KIND=8), ALLOCATABLE    :: Z__DBT(:)        ! Original departures
REAL(KIND=8), ALLOCATABLE    :: Z__Smooth_DBT(:) ! Smoothed departures
REAL(KIND=8), ALLOCATABLE    :: Z__LEVEL(:)      ! Channel height assignments

!* 0.4 Local variables - digital filter parameters
INTEGER(KIND=4)              :: I__CHAN_HIGH          ! Channel at K__Minlev
INTEGER(KIND=4)              :: I__CHAN_LOW           ! Channel at K__Maxlev
INTEGER(KIND=4)              :: I__FirstCloudyChannel ! Highest cloud-affected
                                                      ! channel
INTEGER(KIND=4)              :: I__LastClearChannel   ! Lowest clear channel
INTEGER(KIND=4),POINTER      :: I__Window_Width(:)    ! Box-car filter width
INTEGER(KIND=4),POINTER      :: I__GradChkInterval(:) ! Gradient-check interval

REAL(KIND=8)                 :: Z__Cloud_Level        ! Cloud height
                                                      ! assignment
