...
INTEGER(KIND=4), PARAMETER :: JP__Max_Checks   =  8
INTEGER(KIND=4), PARAMETER :: JP__Max_Channels = 20
...
INTEGER(KIND=4) :: N__Num_Trace_Gas_Checks
INTEGER(KIND=4) :: N__Num_Tracer_Channels(JP__Max_Checks)
INTEGER(KIND=4) :: N__Tracer_Channels(JP__Max_Checks,JP__Max_Channels)
INTEGER(KIND=4) :: N__Num_Control_Channels(JP__Max_Checks)
INTEGER(KIND=4) :: N__Control_Channels(JP__Max_Checks,JP__Max_Channels)
INTEGER(KIND=4) :: N__Num_Flagged_Channels(JP__Max_Checks)
INTEGER(KIND=4) :: N__Flagged_Channels(JP__Max_Checks,JP__Max_Channels)
REAL(KIND=8)    :: R__D_Obs_Threshold(JP__Max_Checks)
REAL(KIND=8)    :: R__D_Dep_Threshold(JP__Max_Checks)
...
WRITE (*,'(A)') ''
WRITE (*,'(A)') 'Setting up the trace gas detection ... '
...
&         (/ 101 /)
   ...
   CASE(INST_ID_HIRAS2)
      ...
      CL__Trace_Gas_Detection_File = 'HIRAS2_TRGASDET.NL'
...
&         (/  21 /)
   ...
   CASE(INST_ID_IRIS)
      ...
      CL__Trace_Gas_Detection_File = 'IRIS_TRGASDET.NL'
...
&         (/ 225 /)
   ...
   OPEN(INIU1,STATUS='OLD',FORM='FORMATTED', &
        FILE=TRIM(CL__Trace_Gas_Detection_File), IOSTAT=IOS)
   IF (IOS == 0) THEN
      READ(INIU1,nml=Trace_Gas_Detect_Coeffs,IOSTAT=IOS)
      IF (IOS == 0) THEN
         WRITE(*,'(3x,A)') TRIM(CL__InstrumentName) // &
              ' TRACE GAS DETECTION FILE READ OK'
      ELSE
         CALL CADS_Abort('PROBLEM READING '//TRIM(CL__InstrumentName)//&
&                 ' TRACE GAS DETECTION FILE')
      ENDIF
      CLOSE(INIU1)
   ELSE
      WRITE(*,'(3x,A)') 'NO '//TRIM(CL__InstrumentName) // &
&              ' TRACE GAS DETECTION FILE : Using Default Values'
   ENDIF
