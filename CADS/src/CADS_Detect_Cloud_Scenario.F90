...
INTEGER(KIND=4), INTENT(IN) :: K__Band             ! Band number
INTEGER(KIND=4), INTENT(IN) :: K__NumChans         ! Number of usable channels
                                                   ! in band
INTEGER(KIND=4), INTENT(IN) :: K__GradChkInterval  ! Gradient-check interval
INTEGER(KIND=4), INTENT(IN) :: K__INDEX(:)         ! Ranking index for dBT
INTEGER(KIND=4), INTENT(IN) :: K__Chan_High        ! First channel clear of high
                                                   ! stratospheric model errors
INTEGER(KIND=4), INTENT(IN) :: K__Chan_Low         ! Last channel clear of PBL
                                                   ! humidity errors
INTEGER(KIND=4), INTENT(IN) :: K__Chan_Windows(2)  ! Two channels defining
                                                   ! long-wave window bounds
INTEGER(KIND=4), INTENT(IN)    :: K__Imager_Flag   ! Input imager cloud flag
INTEGER(KIND=4), INTENT(OUT)   :: K__Scen_Index    ! Choice of scenario
INTEGER(KIND=4), INTENT(OUT)   :: K__Start_Channel ! Final starting channel
REAL(KIND=8), INTENT(IN)       :: P__DBT(:)        ! Input ranked-smoothed
                                                   ! dBT signal

! Local variables
REAL(KIND=8), ALLOCATABLE :: Z__DBT_w_Buffer(:) ! Smoothed-ranked DBT
INTEGER(KIND=4) :: I__Buffer                    ! No. of buffer channels
INTEGER(KIND=4) :: I__Start_Channel             ! Primary starting channel for
                                                ! cloud search
INTEGER(KIND=4) :: I__Start_Channel_Surf        ! Secondary starting channel for
                                                ! cloud search
INTEGER(KIND=4) :: I__Max_Channel               ! Channel corresponding to
                                                ! maximum of the smoothed dBT
INTEGER(KIND=4) :: JCH,JMIN(1),JMAX(1),I
...
REAL(KIND=8)  :: Z__BT_Threshold          ! Solution contaminated threshold
REAL(KIND=8)  :: Z__Grad_Threshold        ! Gradient threshold at which to stop
                                          ! filter procession
REAL(KIND=8)  :: Z__Window_Grad_Threshold ! Gradient threshold for window check
