...
WRITE (*,'(A)') ''
WRITE (*,'(A)') 'Setting up the land sensitivity detection ...'
...
SensorLoop : DO J__Sensor = JP__Min_Sensor_Index, JP__Max_Sensor_Index
   ...
   CASE(INST_ID_HIRAS2)
      ...
      CL__Land_Sensitivity_File = 'HIRAS2_LANDSENS.NL'
      R__Land_Fraction_Thres = 0.5
      R__Level_Thres = 0.9
   ...
   CASE(INST_ID_IRIS)
      ...
      CL__Land_Sensitivity_File = 'IRIS_LANDSENS.NL'
      R__Land_Fraction_Thres = 0.5
      R__Level_Thres = 0.9
   ...
   OPEN(INIU1,STATUS='OLD',FORM='FORMATTED', &
        FILE=TRIM(CL__Land_Sensitivity_File), IOSTAT=IOS)
   IF (IOS == 0) THEN
      READ(INIU1,nml=Land_Sensitivity_Coeffs,IOSTAT=IOS)
      IF (IOS == 0) THEN
         WRITE(*,'(3X,A)') TRIM(CL__InstrumentName) // &
&              ' LAND SENSITIVITY FILE READ OK'
      ELSE
         CALL CADS_Abort('PROBLEM READING '//TRIM(CL__InstrumentName)//&
&                 ' LAND SENSITIVITY FILE')
      ENDIF
      CLOSE(INIU1)
   ELSE
      WRITE(*,'(3X,A)') 'NO '//TRIM(CL__InstrumentName) // &
&              ' LAND SENSITIVITY FILE : Using Default Values'
   ENDIF
