...
USE CADS_Module, ONLY : S__CADS_Setup_Land

IMPLICIT NONE

! Global arrays
INTEGER(KIND=4),INTENT(IN)  :: K__Sensor
INTEGER(KIND=4),INTENT(IN)  :: K__NumChans
REAL(KIND=8)   ,INTENT(IN)  :: P__Land_Fraction
REAL(KIND=8)   ,INTENT(IN)  :: P__CldLev(:)
INTEGER(KIND=4),INTENT(OUT) :: K__LandSens_Flag(:)

! Local variables
REAL(KIND=8), ALLOCATABLE :: Z__CldLev_NORM(:)
REAL(KIND=8) :: Z__Land_Thres
REAL(KIND=8) :: Z__Level_Thres
REAL(KIND=8) :: Z__Maxlev
